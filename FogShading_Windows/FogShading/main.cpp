...
static const int TEAPOT_COUNT = 3;
static const float SPACING = 16.0f;
...
void g_init() {
    ...
    mesh->loadShader("shaders/FogShading.vert", "shaders/FogShading.frag");
    ...
    fog.maxDist = 140.0f;
    fog.minDist = 20.0f;
    fog.color = Vector3f(1.0f, 1.0f, 1.0f);
}
...
void g_glutDisplayFunc() {
    ...
    for ( int i = -TEAPOT_COUNT + 1; i < TEAPOT_COUNT; i++ ) {
        for ( int j = -TEAPOT_COUNT + 1; j < TEAPOT_COUNT; j++ ) {
            mesh->setPosition(static_cast<float>(i) * SPACING, 0.0f, static_cast<float>(j) * SPACING);
            ...
            mesh->getShader()->uniformVector("lightPosition", Vector3f(0.0f, 1.0f, -10.0f));
            mesh->getShader()->uniform1f("fog.minDist", fog.minDist);
            mesh->getShader()->uniform1f("fog.maxDist", fog.maxDist);
            mesh->getShader()->uniformVector("fog.color", fog.color);
            mesh->endRender();
        }
    }
