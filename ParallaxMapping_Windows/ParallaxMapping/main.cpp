...
void g_init() {
    ...
    camera->setPosition(18.0f, 1.1707f, 1.970f * 0.7f);

    mesh = std::make_shared<Mesh>();
    mesh->load("models/cube.obj");
    mesh->loadShader("shaders/ParallaxMapping.vert", "shaders/ParallaxMapping.frag");

    mesh->setDiffuseTexture("textures/diffuse_texture.png");
    mesh->setHeightmapTexture("textures/heightmap_texture.png");
}
...
void g_glutDisplayFunc() {
    ...
    mesh->getShader()->uniform1f("scale", 0.04f);
    mesh->getShader()->uniform1f("bias", 0.04f);
    mesh->endRender();
