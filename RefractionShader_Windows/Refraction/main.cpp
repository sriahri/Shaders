...
void g_init() {
    ...
    mesh->loadShader("shaders/Refraction.vert", "shaders/Refraction.frag");

    // Simple illustration for quickly loading some geometry for a 'skybox' effect.
    skybox = std::make_shared<Mesh>();
...
void g_glutDisplayFunc() {
    ...
    skybox->beginRender();
		skybox->getShader()->uniformMatrix("projectionMatrix", projectionMatrix);
		skybox->getShader()->uniformMatrix("modelViewMatrix", modelViewMatrix);
		skybox->getShader()->uniformMatrix("normalMatrix", normalMatrix);
		skybox->getShader()->uniformVector("cameraPosition", camera->getEye());
    skybox->endRender();
